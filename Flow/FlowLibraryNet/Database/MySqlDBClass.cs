...
namespace FlowLibraryNet.Database
{
    ...
    public class MySqlDBClass : Shotgun.Database.BDClass, Shotgun.Database.IBaseDataSpecial
    {
        ...
        {
            string table;
            if (string.IsNullOrEmpty(DataRow.Schema))
                table = string.Format("`{0}`", DataRow.TableName);
            else
                table = string.Format("`{0}`.`{1}`", DataRow.Schema, DataRow.TableName);
        ...
        public Shotgun.Model.List.EM_PAGE_Mode PageMode
        {
            get { return Shotgun.Model.List.EM_PAGE_Mode.ByLimit; }
        }

        public Shotgun.Model.Filter.EM_Safe_Field_MASK FieldMask
        {
            get { return Shotgun.Model.Filter.EM_Safe_Field_MASK.MySQLMode; }
        }

        public string funcNullToValue(string field, object defaultValue)
        {
            return string.Format("ifnull({0},'{1}')", FieldEncode(field), defaultValue);
        }
