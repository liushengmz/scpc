...
public class TroneDao
{
	...
	{
		String sql = "SELECT a.pro_id,a.ratio ratio,b.send_sms,a.id trone_id,";
		sql += " b.id sp_trone_id,e.id sp_id,c.num flow_size,c.operator,c.price,d.use_rang rang,d.time_type";
		sql += " FROM daily_config.tbl_f_trone a";
		sql += " LEFT JOIN daily_config.tbl_f_sp_trone b ON a.sp_trone_id = b.id";
		sql += " LEFT JOIN daily_config.tbl_f_basic_price c ON b.price_id = c.id";
		sql += " LEFT JOIN daily_config.tbl_f_flow_type d ON b.flow_type_id = d.id";
		sql += " LEFT JOIN daily_config.tbl_f_sp e ON b.sp_id = e.id ";
