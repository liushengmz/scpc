...
a.util={
      ...
      getValue:function(m){
            ...
            if(m.is(":radio")){
                  ...
                  if(m.is(":checkbox")){
                        l=k.find(":checkbox[name='"+m.attr("name")+"']:checked").val();
                        l=l===b?"":l
                        }else{
                        l=m.val()
                        }
...
enhance:function(){
      var k=this;
      k.find("input[recheck]").each(function(){
            if(this.validform_inited=="inited"){
                  return true
                  }
                  this.validform_inited="inited";
...
k.find(":checkbox[datatype],:radio[datatype]").each(function(){
      if(this.validform_inited=="inited"){
            return true
            }
            this.validform_inited="inited";
      var m=d(this);
      var l=m.attr("name");
      k.find("[name='"+l+"']").filter(":checkbox,:radio").bind("click",function(){
            setTimeout(function(){
                  m.trigger("blur")
                  },0)
            })
...
if(s.datepicker&&d.fn.datePicker){
      ...
      s.datepicker.callback&&r.bind("dateSelected",function(){
            var n=new Date(d.event._dpCache[this._dpId].getSelected()[0]).asString(Date.format);
      ...
      v.find("input[plugin*='passwordStrength']").passwordStrength(s.passwordstrength)
      }
      if(s.jqtransform&&d.fn.jqTransSelect){
      var m=function(n){
            var w=d(".jqTransformSelectWrapper ul:visible");
            w.each(function(){
                  var x=d(this).parents(".jqTransformSelectWrapper:first").find("select").get(0);
...
if(s.jqtransform.selector){
      v.find(s.jqtransform.selector).filter('input:submit, input:reset, input[type="button"]').jqTransInputButton();
      v.find(s.jqtransform.selector).filter("input:text, input:password").jqTransInputText();
      v.find(s.jqtransform.selector).filter("input:checkbox").jqTransCheckBox();
      v.find(s.jqtransform.selector).filter("input:radio").jqTransRadio();
      v.find(s.jqtransform.selector).filter("textarea").jqTransTextarea();
      if(v.find(s.jqtransform.selector).filter("select").length>0){
            v.find(s.jqtransform.selector).filter("select").jqTransSelect();
            u()
            }
      }else{
      v.jqTransform()
      }
      v.find(".jqTransformSelectWrapper").find("li a").click(function(){
...
_regcheck:function(r,x,q,y){
      ...
      if(p===true||p===b){
            ...
            n=y.data("tipmsg").r||e.r;
            if(q.attr("recheck")){
                  var l=y.find("input[name='"+q.attr("recheck")+"']:first");
                  if(x!=l.val()){
                        p=false;
                        w=3;
                        n=q.attr("errormsg")||y.data("tipmsg").w||e.w
      ...
      if(x===""){
            return{
                  passed:false,
                  type:3,
                  info:q.attr("nullmsg")||y.data("tipmsg").s||e.s
                  }
            }
}
return{
...
if(!(r in a.util.dataType)){
      ...
      if(!u){
            ...
            if(u[1]===z[1]){
                  var v=a.util.dataType[k].toString(),o=v.match(/\/[mgi]*/g)[1].replace("/",""),s=new RegExp("\\{"+z[2]+","+z[3]+"\\}","g");
                  v=v.replace(/\/[mgi]*/g,"/").replace(s,"{"+u[2]+","+u[3]+"}").replace(/^\//,"").replace(/\/$/,"");
...
check:function(u,p,t,m){
      ...
      if(k==this.validform_lastval&&!d(this).attr("recheck")||d(this).data("dataIgnore")==="dataIgnore"){
            return true
            }
            if(n.dragonfly&&!d(this).data("cked")&&a.util.isEmpty.call(d(this),k)){
            return false
            }
      ...
      if(!r.passed){
            ...
            if(d(this).attr("ajaxurl")&&!m&&d(this).attr("ignore")!="ignore"){
                  var l=d(this);
                  if(l[0].validform_valid==="posting"&&k==l[0].validform_ckvalue){
                        return"ajax"
                        }
                  ...
                  q[0].validform_ajax=d.ajax({
                        ...
                        cache:false,
                        url:o,
                        data:a.util.getQuery.call(f,o)+"param="+k+"&name="+d(this).attr("name"),
                        dataType:"text",
                        success:function(v){
                              if(d.trim(v)=="y"){
                                    l[0].validform_valid="true";
                                    a.util.showmsg.call(s,p.tipmsg.r||e.r,n.tiptype,{
                                          obj:l,
                                          type:2,
                                          sweep:n.tipSweep
                                          },"hide");
                  ...
                  error:function(v){
                        if(v.statusText!=="abort"){
                              q.addClass("Validform_error");
                              a.util.showmsg.call(s,p.tipmsg.err||e.err,n.tiptype,{
                                    obj:l,
                                    type:3,
                                    sweep:n.tipSweep
                                    })
...
submitForm:function(m,k,o,q){
      ...
      s.find("[datatype]").each(function(){
            if(k){
                  ...
                  if(m.ignoreHidden&&d(this).is(":hidden")||d(this).data("dataIgnore")==="dataIgnore"){
                  return true
                  }
                  var u=a.util.getValue.call(s,d(this)),v;
            g=v=d(this);
            l=a.util.regcheck.call(s,d(this).attr("datatype"),u,d(this));
            if(!l.passed){
                  ...
                  if(d(this).attr("ajaxurl")&&d(this).attr("ignore")!="ignore"){
                  if(this.validform_valid!=="true"){
                        var t=d(this);
                        v.addClass("Validform_error");
                        a.util.showmsg.call(s,s.data("tipmsg").v||e.v,m.tiptype,{
                              obj:t,
                              type:3,
                              sweep:m.tipSweep
                              });
...
if(m.showAllError){
      ...
      if(m.ajaxPost||o==="ajaxPost"){
            a.util.showmsg.call(s,s.data("tipmsg").p||e.p,m.tiptype,{
                  obj:s,
                  type:1,
                  sweep:m.tipSweep
                  },"alwaysshow");
            s[0].validform_ajax=d.ajax({
                  type:"POST",
                  dataType:"json",
                  async:q,
                  url:s.attr("action"),
                  ...
                  success:function(t){
                        if(t.status==="y"){
                              ...
                              a.util.showmsg.call(s,t.info,m.tiptype,{
                                    obj:s,
                                    type:2,
                                    sweep:m.tipSweep
                                    },"alwaysshow")
                  ...
                  error:function(t){
                        var u=t.statusText==="abort"?s.data("tipmsg").abort||e.abort:s.data("tipmsg").err||e.err;
...
ajaxPost:function(k,l){
      var m=this;
      if(m.settings.tiptype==1||m.settings.tiptype==2||m.settings.tiptype==3){
            c()
            }
