...
<%
	...
	int groupId = StringUtil.getInteger(ConfigManager.getConfigData("AD_CP_CPS_GROUP_ID"),-1);
	
	AdAppModel app = new AdAppServer().loadAppById(id);
...
<script type="text/javascript">
	...
	function goToMain()
	{
		window.location.href = "app.jsp?<%= Base64UTF.decode(query) %>";
	}
	
	$(function()
	{
		setRadioCheck("userid",<%= app.getUser_id() %>);
		<%= msg %>
	});
		...
		<div class="content" style="margin-top: 10px">
			<dl>
				<dd class="ddbtn" style="width:500px">
					<label>广告CP登录分配(<%= app.getAppname() %>)</label>
				</dd>
			</dl>
			<br /> <br />
			<dl>
				<form action="appaction.jsp?query=<%= query %>" method="post" id="addform">
