...
<%
	...
	CpChannelModel model = new CpChannelServer().loadCpChannelById(id);
	...
	String channelid = StringUtil.getString(request.getParameter("channelidstr"), "");	
	...
	$(function()
			{
				...
				%>
				$("#input_scale").val("<%=num%>");
				setRadioCheck("type2",<%= model.getStatus()%>);
			});
...
<body>
	<div class="main_content">
		<div class="content" style="margin-top: 10px">
			...
			<br />	<br />		
			<dl>
				<form action="cpqdaction.jsp?startdate=<%=startDate %>&enddate=<%=endDate %>&appnamestr=<%=appid %>&channelidstr=<%=channelid %>" method="post" id="addform">
				<input type="hidden" value="<%= model.getId() %>" name="id" />
					<input type="hidden" value="<%= StringUtil.getInteger(request.getParameter("pageindex"), 1) %>" 
					  name="pageindex" />
					<dd class="dd00_me"></dd>
					...
					<dd class="dd03_me">
						<input id="type" type="radio" name="type2" style="width: 35px;float:left" value="0" checked="checked" >
