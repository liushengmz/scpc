...
<%
	...
	ChannelModel channel = new ChannelServer().loadQdById(id);
	...
	int groupId = StringUtil.getInteger(ConfigManager.getConfigData(channel.getSettleType()==1 ? "GAME_QD_CPA_GROUP_ID" : "GAME_QD_CPS_GROUP_ID"),-1);
...
<script type="text/javascript">
	...
	function goToMain()
	{
		window.location.href = "channel.jsp?<%= Base64UTF.decode(query) %>";
	}
	
	$(function()
	{
		setRadioCheck("userid",<%= channel.getUserid() %>);
		<%= msg %>
	});
		...
		<div class="content" style="margin-top: 10px">
			<dl>
				<dd class="ddbtn" style="width:500px">
					<label>游戏渠道<%= channel.getSettleType()==1 ? "CPA" : "CPS" %>登录分配(<%= channel.getAppname() + "[" + channel.getChannel() + "]" %>)</label>
				</dd>
			</dl>
			<br /> <br />
			<dl>
				<form action="qdaction.jsp?query=<%= query %>" method="post" id="addform">
					<input type="hidden" value="<%= id %>" name="id">
					<input type="hidden" value="1" name="atype">
