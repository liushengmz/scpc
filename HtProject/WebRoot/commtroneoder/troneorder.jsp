...
<%
	...
	Map<String, Object> map =  new TroneOrderServer().loadTroneOrder(spId, spTroneId, cpId,status,pageIndex,keyWord,userId);
...
<body>
	<div class="main_content">
		<div class="content" >
			<dl>
				<dd class="ddbtn" ><a href="troneorderadd.jsp" onclick="return checkAdd()">增  加</a></dd>
			</dl>
		...
		<table cellpadding="0" cellspacing="0">
			...
			<tbody>
				...
				<tr <%= model.getDisable() == 1 ? stopStyle : "" %>>
					...
					<td>
						<a href="troneorderedit.jsp?query=<%= query %>&id=<%= model.getId() %>" onclick="return checkUser('<%=model.getCommerceUserId()%>')">修改</a>
						<a href="troneordersync.jsp?id=<%= model.getId() %>" target="_blank"  onclick="return checkUser('<%=model.getCommerceUserId()%>')">模拟</a>
					</td>
				</tr>
				<%
					}
