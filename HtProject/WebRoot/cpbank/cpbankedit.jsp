...
<%
	...
	CpBankModel model = new CpBankInfoServer().loadCpBankById(id);
	if(model==null)
	{
		response.sendRedirect("cpbank.jsp");
		return;
	}
...
%>
	...
	function resetForm()
	{
		$("#cp_id").val("<%=model.getCpId()%>");
		$("#bank_name").val("<%=model.getBankName()%>");
		$("#user_name").val("<%=model.getUserName()%>");
		$("#bank_account").val("<%=model.getBankAccount()%>");
		$("#bank_branch").val("<%=model.getBankBranch()%>");
	...
	function subForm() 
	{
		...
		var id='<%=id%>';
		var result=ajaxLogin(cpId, type,status,id);		
		if(result=='1'){
			alert("数据已存在！");
			return ;
		}
	...
	function ajaxLogin(cpId, type,status,id) {
		var result = "";
		$.ajax({
			url : "action.jsp",
			data : "cp_id=" + cpId + "&type=" + type+"&status="+status+"&id="+id+"&act=2",
			cache : false,
			async : false,
				...
				<form action="action.jsp?query=<%= query %>" method="post"  id="addform">
					...
					<dd class="dd04_me">
						<select name="cp_id" id="cp_id" style="width: 200px" onclick="namePicker(this,cpList,onCpDataSelect)">
							...
							{
								%>
							<option value="<%= cpModel.getId() %>"><%= cpModel.getShortName() %></option>	
								<%
							}
							%>
						</select>
