...
<script type="text/javascript">
	...
	function showConfirmCwDialog(id,type)
	{	
		$("#lab_cw_title").text($("#lab_sp_name_" + id).text() + "[" + $("#lab_start_date_" + id).text() + "至" + $("#lab_end_date_" + id).text() + "][" + $("#lab_js_name_" + id).text() + "]");
  		...
  		$("#lab_cwamount").text($("#lab_amount_" + id).text());
  		$("#lab_prebilling").text($("#lab_pre_billing_" + id).text());
  		...
  		if(type==3){
  			...
  			if(dateArray[2]==""||null==dateArray[2]){
  		  		$("#lab_kaipiao_year").val('<%=defDate[0]%>');
  		  		$("#lab_kaipiao_month").val('<%=defDate[1]%>');
  		  		$("#lab_kaipiao_day").val('<%=defDate[2]%>');
  			}else{
  				var splitDate=getYearMonthDayByDate(dateArray[2]);
  		  		$("#lab_kaipiao_year").val(splitDate[0]);
  		  		$("#lab_kaipiao_month").val(splitDate[1]);
  		...
  		$("#btn_cw_confirm").click(function(){
  			...
  			//财务开票操作，更改状态和时间
  			if(type==3){  
  				var kaipiaoyear=$("#lab_kaipiao_year").val();
  				var kaipiaomonth=$("#lab_kaipiao_month").val();
  				var kaipiaoday=$("#lab_kaipiao_day").val();
				var date=kaipiaoyear+"-"+kaipiaomonth+"-"+kaipiaoday;
  				ajaxGetgetSpBillingDate(id,type,5,date);
  				$( "#cwdialog" ).dialog("close");
	...
	function confirmActureBilling(id)
	{
		...
		getAjaxValue("action.jsp?type=4&id=" + id + "&date="+date+"&money=" + actureBilling,onConfirmSpBilling);
...
<body style="min-height: 800px;padding-top: 40px">
	...
	<div id="cwdialog" title="账单明细" style="">
  		<label id="lab_cw_title" >账单明细</label>
  		<br />
  		...
  		<br />
  		  信息费 ：<label id="lab_cwamount">123456</label>
  		<br />
  		...
  		<input id="btn_cw_confirm" style="float: right;font-size: 14px;font-weight: bold;cursor: pointer;" type="button" value="确认" >
	</div>
</body>
</html>
