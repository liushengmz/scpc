...
<%
	...
	Map<String, Object> map =  new LvMrServer().getLvMrTodayData(date, payType, userId);
	List<LvMrModel> list=(List<LvMrModel>)map.get("list");
	float dayAmount=(Float)map.get("dayAmount");
...
<body>
	<div class="main_content">
		<div class="content" >
			<form action="lvmr2.jsp"  method="get">
				<dl>
		...
		<table cellpadding="0" cellspacing="0" id="table_id">
			<thead>
				<tr>
					<td>序号</td>
					<td onclick="TableSorter('table_id',2,'String')">订单号</td>
					<td >支付方式</td>
					<td onclick="TableSorter('table_id',4,'float')">价格(分)</td>
					<td onclick="TableSorter('table_id',5,'date')">订单日期</td>
			...
			<tbody>		
				...
				<tr>
					<td><%= index++ %></td>
					<td><%=model.getOrderId() %></td>
					<td><%=model.getPayType()==1?"支付宝":"微信" %></td>
			...
			<tbody>
				<tr>
					...
					<td></td>
					<td></td>
					<td>总额(元):<%=StringUtil.getDecimalFormat(dayAmount) %> </td>
					<td></td>
				</tr>
			</tbody>
		</table>
