...
public class RightAccessDao
{
	...
	{
		String sql = "SELECT a.id userId,f.`id` headId,e.id menu1Id,d.id menu2Id FROM " + Constant.DB_DAILY_CONFIG + ".tbl_user a "
				+ "LEFT JOIN " + Constant.DB_DAILY_CONFIG + ".tbl_group_user b ON a.id = b.`user_id` "
				+ "LEFT JOIN " + Constant.DB_DAILY_CONFIG + ".tbl_group_right c ON c.`group_id` = b.`group_id` "
				+ "LEFT JOIN " + Constant.DB_DAILY_CONFIG + ".tbl_menu_2 d ON d.`id` = c.`menu_2_id` "
				+ "LEFT JOIN " + Constant.DB_DAILY_CONFIG + ".tbl_menu_1 e ON d.`menu_1_id` = e.`id` "
				+ "LEFT JOIN " + Constant.DB_DAILY_CONFIG + ".tbl_menu_head f ON e.`head_id` = f.`id` "
