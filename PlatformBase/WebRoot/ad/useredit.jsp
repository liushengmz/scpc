...
<%
	...
	SystemUserModel model = new SystemUserServer().loadUserById(userId);
...
<script type="text/javascript">
	...
	function resetForm()
	{
		$("#input_nick_name").val("<%= model.getNick_name() %>");
		$("#input_mail").val("<%= model.getMail() %>");
		$("#input_qq").val("<%= model.getQq() %>");
		$("#input_phone").val("<%= model.getPhone() %>");
		$("#input_new_pwd_1").val("<%= model.getPwd() %>");
		$("#input_new_pwd_2").val("<%= model.getPwd() %>");
...
<body>
	<div class="main_content">
		<div class="content" style="margin-top: 10px">
			...
			<br />	<br />
			<dl>
				<form action="useraction.jsp" method="post" id="addform">
				<input type="hidden" value="<%= userId %>" name="id" />
				<input type="hidden" value="<%= model.getPwd() %>" name="old_pwd">
				...
				<dd class="dd03_me">
					<input type="password" name="pwd"   id="input_new_pwd_1" value="<%= model.getPwd() %>" style="width: 200px">
				</dd>
				...
				<dd class="dd03_me">
					<input type="password" name="new_pwd_2"  id="input_new_pwd_2" value="<%= model.getPwd() %>" style="width: 200px">
				</dd>
				...
				<dd class="dd03_me">
					<input type="text" name="nick_name"  id="input_nick_name"  value="<%= model.getNick_name() %>" style="width: 200px">
				</dd>
