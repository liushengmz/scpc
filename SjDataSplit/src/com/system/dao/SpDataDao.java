...
public class SpDataDao
{
	...
	{
		String sql = "SELECT  a.*,b.short_name,d.operator_id,'name_cn' name_cn";
		sql += " FROM yd_bak.tbl_sp_trone a";
		sql += " LEFT JOIN yd_bak.tbl_sp b ON a.sp_id = b.id";
		sql += " LEFT JOIN yd_bak.tbl_product_2 c ON a.product_id = c.id";
		sql += " LEFT JOIN yd_bak.tbl_product_1 d ON c.product_1_id =d.id;";
		...
		return (List<SpTroneModel>)new JdbcControl().query(sql, new QueryCallBack()
		{
			...
			public Object onCallBack(ResultSet rs) throws SQLException
			{
				...
				while(rs.next())
				{
					...
					model.setOperator(rs.getInt("operator_id"));
					...
					model.setOperator(model.getOperator()<=0 ? 1 : model.getOperator());
	...
	{
		String sql = "select a.*,b.short_name from " + com.sytem.dd.DddFord.dataBase + ".tbl_trone a left join " + com.sytem.dd.DddFord.dataBase + ".tbl_sp b on a.sp_id = b.id order by b.short_name asc";
