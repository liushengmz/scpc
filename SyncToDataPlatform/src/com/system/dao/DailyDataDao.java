...
public class DailyDataDao
{
	...
	private StringBuffer loadSqlByType(String feeDate,int recordType)
	{
		...
		sb.append(" LEFT JOIN daily_config.tbl_trone_order b ON a.trone_order_id = b.id");
		sb.append(" LEFT JOIN daily_config.tbl_trone c ON a.`trone_id` = c.`id`");
		sb.append(" LEFT JOIN daily_config.tbl_sp_trone d ON c.`sp_trone_id` = d.id");
		sb.append(" LEFT JOIN daily_config.tbl_sp e ON d.`sp_id` = e.`id`");
		sb.append(" LEFT JOIN daily_config.tbl_cp g ON b.`cp_id` = g.`id`");
		sb.append(" LEFT JOIN daily_config.tbl_province h ON a.province_id = h.id");
		sb.append(" LEFT JOIN daily_config.`tbl_product_2` i ON d.`product_id` = i.`id`");
		sb.append(" LEFT JOIN daily_config.`tbl_product_1` j ON i.`product_1_id` = j.`id`");
		sb.append(" LEFT JOIN daily_config.`tbl_operator` k ON j.`operator_id` = k.id ");
