 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class Report1036{
	private static Logger logger = Logger.getLogger(Report.class);

	private String sql="";


	public Report1036(Statement db)
	{
		logger.debug("now begin Report1036==斯美通");
		checkReport(db);

	}

	private void checkReport(Statement db) {
		sql="select service_code,exchange_id,messageid,src_phone,dst_phone,exchange_time,success_time,exchange_status  from mms_transaction_detail where exchange_status<>1 and report_status=0 and linkid!='' and league_id='1036' order by exchange_id desc limit 200";
		try {	
			...
			if (!updatesql.equals("")) {
				try {
					...
					logger.debug("1036Report res:"+res);
					if (res.equals("0")) {
