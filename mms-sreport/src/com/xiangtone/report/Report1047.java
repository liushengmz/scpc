 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class Report1047 {
	private static Logger logger = Logger.getLogger(Report.class);

	private String sql = "";
	// http://211.154.135.192:51414/ppgwp/szcshdcxmot.jsp
	private String leagueid = "1047";

	public Report1047(Statement db) {
		logger.debug("now begin Report1047===X-Íß¶ûÀ×Ë¼");
		checkReport(db);
	}
	...
	private void checkReport(Statement db) {
		...
		try {
			...
			sb.append("<?xml version=\"1.0\"  encoding=\"GBK\" ?><report>");
			while (rs.next()) {
			...
			if (!updatesql.equals("")) {
				try {
					...
					// http://211.154.135.181:81/ppgwp/szcshdydcxmot.jsp
					// http://211.154.135.192:51414/ppgwp/szcshdcxmot.jsp
					String res = send("http://211.154.135.181:81/ppgwp/szcshdydcxmot.jsp", sb);

					// http://119.147.24.103:8080/channel/cshd2
					logger.debug("1047Report res:" + res);
					if (res.equals("0")) {
