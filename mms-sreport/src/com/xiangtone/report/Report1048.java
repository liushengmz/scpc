 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class Report1048 {
	private static Logger logger = Logger.getLogger(Report.class);

	private String sql = "";
	private String leagueid = "1048";

	public Report1048(Statement db) {
		logger.debug("now begin Report1048===Ê¢²©ÌØ");
		checkReport(db);
		...
		try {
			...
			if (!updatesql.equals("")) {
				try {
					...
					String res = send("http://www.cpu009.com/service/xtxx.php", sb);
					// http://119.147.24.103:8080/channel/cshd2
					logger.debug("1048Report res:" + res);
					if (res.equals("0")) {
