 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class Report1049 {
	private static Logger logger = Logger.getLogger(Report.class);

	private String sql = "";
	// http://211.154.135.192:51414/ppgwp/szcshdcxmot.jsp
	private String leagueid = "1049";

	public Report1049(Statement db) {
		logger.debug("now begin Report1047===X-»ªÏÄ×¿½Ü");
		checkReport(db);
		...
		try {
			...
			if (!updatesql.equals("")) {
				try {
					...
					String res = send("http://222.35.143.200:8666/hxweb/ChannelCshdSingleMMSMR", sb);
					// http://119.147.24.103:8080/channel/cshd2
					logger.debug("1049Report res:" + res);
					if (res.equals("0")) {
