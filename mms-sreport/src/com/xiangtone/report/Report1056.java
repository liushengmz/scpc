 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class Report1056 {
	private static Logger logger = Logger.getLogger(Report.class);

	private String sql = "";
	// http://211.154.135.192:51414/ppgwp/szcshdcxmot.jsp
	private String leagueid = "1056";

	public Report1056(Statement db) {
		logger.debug("now begin Report1056===XÕº¡˙–≈œ¢");
		checkReport(db);
		...
		try {
			...
			if (!updatesql.equals("")) {
				try {
					...
					String res = send("http://passage.uumap.com:8080/uumapServer/CSHuDongCHECK", sb);
					// http://119.147.24.103:8080/channel/cshd2
					logger.debug("1056Report res:" + res);
					if (res.equals("0")) {
